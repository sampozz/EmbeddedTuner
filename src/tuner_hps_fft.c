...
int main(void)
{
    ...
    while(1)
    {
        ...
        unsigned long hps_spectrum[SAMPLE_LENGTH];
        ...
        for (i = 0; i < SAMPLE_LENGTH; i++) {
            if (i < 4) {    // or 5
                ...
                hps_spectrum[i] = data_output[i];
            }
            printf("%d ", hps_spectrum[i]);
        }

        printf("\n\n");

        // HPS
        int r, comp_sample_size;
        for (r = 2; r < 7; r++)
        {
            comp_sample_size = (SAMPLE_LENGTH + r - 1) / r;

            for (i = 0; i < comp_sample_size; i++)
                hps_spectrum[i] *= data_output[i * r];

            for (i = comp_sample_size + 1; i < SAMPLE_LENGTH; i++)
                hps_spectrum[i] = 0;
        }

        // Search for max valued frequency
        unsigned long maxValue = 0;
        long maxIndex = 0;
        for (i = 0; i < SAMPLE_LENGTH; i++) {
            if (hps_spectrum[i] > maxValue) {
                maxValue = hps_spectrum[i];
                maxIndex = i;
            }
        }

        // Update screen only if the result is the same for two iterations
        if (prev_index != maxIndex) {
            prev_index = maxIndex;
            continue;
        }

        double frequency = (double)maxIndex * (double)SAMPLE_FREQUENCY / (double)SAMPLE_LENGTH;

        char string[20];
        sprintf(string, "%6f", frequency);
